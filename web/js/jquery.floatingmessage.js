...
( function( $ ) {
    var ranges = {
        ...
        remove = function( elem ) {
            ...
            elem.stop().fadeOut( duration, function() {
                $( this ).remove();
                if ( close ) close();
            } );
        };

    $.floatingMessage = function( message, options, alertType ) {
        var id = "jqueryFloatingMessage" + new Date().getTime() + "" + parseInt( Math.random() * 10000, 10 ),
            elem = $( '<div id="'+id+'" class="alert alert-' + alertType + ' ui-widget-content ui-corner-all ui-floating-message"></div>').hide(),
            css = {},
            height = 0,
            padding = 0,
            open;
        ...
        elem.fadeIn( options.duration,function(){
            if ( options.time ) {
